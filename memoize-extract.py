...
__version__ = '2023/12/01 v1.1.0-wip'

import argparse, re, sys, os, subprocess, itertools, traceback, platform
from pathlib import Path, PurePath
# We will only try to import the PDF processing library once we set up the error log.
...
ERROR = {
    'latex':   r'\PackageError{{memoize (python-based extraction)}}{{{short}}}{{{long}}}',
    'plain':   r'\errhelp{{{long}}}\errmessage{{memoize (python-based extraction): {short}}}',
    'context': r'\errhelp{{{long}}}\errmessage{{memoize (python-based extraction): {short}}}',
    None:      '{header}{short}.\n{long}',
}

WARNING = {
    'latex':   r'\PackageWarning{{memoize (python-based extraction)}}{{{texindent}{text}}}',
    'plain':   r'\message{{memoize (python-based extraction): {texindent}{text}}}',
    'context': r'\message{{memoize (python-based extraction): {texindent}{text}}}',
    None:      r'{header}{indent}{text}.',
}

INFO = {
    'latex':   r'\PackageInfo{{memoize (python-based extraction)}}{{{texindent}{text}}}',
    'plain':   r'\message{{memoize (python-based extraction): {texindent}{text}}}',
    'context': r'\message{{memoize (python-based extraction): {texindent}{text}}}',
    None:      r'{header}{indent}{text}.',    
}
...
def error(short, long):
    ...
    if log:
        ...
        print(ERROR[args.format].format(short = short, long = long), file = log)
    global exit_code
    exit_code = 11
    endinput()
...
def warning(text):
    if log:
        print(WARNING[args.format].format(text = text, texindent = texindent), file = log)
    if not args.quiet:
...
def info(text):
    if text and not args.quiet:
        ...
        if log:
            print(INFO[args.format].format(text = text, texindent = texindent), file = log)
...
def paranoia_in(f, remark = ''):
    if f and not _paranoia(f, openin_any):
        error(f"I'm not allowed to read from '{f}' (openin_any = {openin_any})", remark)

def paranoia_out(f, remark = ''):
    if f and not _paranoia(f, openout_any):
        error(f"I'm not allowed to write to '{f}' (openout_any = {openout_any})", remark)

def _paranoia(f, mode):
    return (mode in 'ay1'
            or (not (os.name == 'posix' and f.stem.startswith('.') and not f.stem == '.tex')
                and
                (mode in 'rn0'
                 or
                 ('..' not in f.parts
...
# This function can fail on Windows, reporting a non-writable file or dir as writable,
# because |os.access| does not work with Windows' |icacls| permissions.  Consequence: we
# might try to write to a read-only current or output directory instead of switching to
# the temporary directory.  Paranoia is unaffected, as it doesn't use |access_*|
# functions.
def access_out(f):
    try:
        exists = f.exists()
...
# This function assumes that both paths are absolute; ancestor may be |None|, signaling a
# non-path.
...
if __name__ == '__main__':
    ...
    parser.add_argument('-m', '--mkdir', action = 'store_true',
        help = 'create a directory (and exit); mmz argument is interpreted as directory name')
    ...
    try:
        ...
        try:
            ...
            info(f"File '{given_mmz_file}' does not exist, assuming there's nothing to do")
            endinput()

        # Determine the PDF filename: it is either given via |--pdf|, or constructed from
        # the |.mmz| filename.
        pdf_file = find_in(Path(args.pdf) if args.pdf else given_mmz_file.with_suffix('.pdf'))
        paranoia_in(pdf_file)
        if args.prune:
            paranoia_out(pdf_file,
                remark = 'I would have to rewrite this file because option --prune was given.')
        ...
        re_extern_path = re.compile(
            ...
            r'(?P<code_md5sum>[0-9A-F]{32})-(?P<context_md5sum>[0-9A-F]{32})(?:-[0-9]+)?.pdf')
        pdf = None
        extern_pages = []
        new_mmz = []
        tolerance = 0.01
        ...
        for line in mmz:
            try:
                if m_p := re_prefix.match(line):
                    # Found |\mmzPrefix|: create the extern directory, but only later, if
                    # an extern file is actually produced.  We parse the prefix in two
                    # steps because we have to unquote the entire prefix.
                    ...
                    c_memo = extern_file.with_name(m_ep['name_prefix'] + m_ep['code_md5sum'] + '.memo')
                    cc_memo = extern_file.with_name(m_ep['name_prefix'] +
                                m_ep['code_md5sum'] + '-' + m_ep['context_md5sum'] + '.memo')
                    c_memo_in = find_in(c_memo)
                    cc_memo_in = find_in(cc_memo)
                    if not (access_in(c_memo_in) and access_in(cc_memo_in)) and not args.force:
                        warning(f"I refuse to extract page {page_n+1} into extern "
                                f"'{extern_file}', because the associated c-memo "
                                f"'{c_memo}' and/or cc-memo '{cc_memo}' does not exist")
                        raise NotExtracted()
                    # Load the PDF.  We only do this now so that we don't load it if there is
                    # nothing to extract.
                    if not pdf:
                        ...
                        try:
                            ...
                            error(rf"File '{pdf_file}' seems corrupted. "
                                  rf"Perhaps you have to load Memoize earlier in the preamble",
                                  f"In particular, Memoize must be loaded before TikZ library "
                                  f"'fadings' and any package deploying it, and in Beamer, "
                                  f"load Memoize by writing \RequirePackage{{memoize}} before "
                    ...
                    if (abs(width_pt - expected_width_pt) > tolerance
                       or abs(height_pt - expected_height_pt) > tolerance) and not args.force:
                        warning(f"I refuse to extract page {page_n+1} from '{pdf_file}' "
                                f"because its size ({width_pt}pt x {height_pt}pt) "
                                f"is not what I expected "
                                f"({expected_width_pt}pt x {expected_height_pt}pt)")
                        raise NotExtracted()
                    # All tests were successful, let's create the extern file.  First, the
