...
\AddToHook{begindocument}{% so that the region compilation takes these in
  \docForeign{
    ...
    scope={
      before/.style={index annotation=\hologo{XeTeX} primitive},
      cmd={name=leavevmode},
    },
    ...
    file={name=memoize-doc.sty, index annotation=the Memoize manual style file},
  }%
  \docindex{%
...
\begin{tcolorbox}[emph, fontupper=\footnotesize]
  \mmzUnmemoizable
...
\begin{tcboxedraster}[raster columns=4, raster valign=bottom]{blankest}
...
~\mmzset~{
  ...
  padding=2in}
  \end{tcblisting}
\end{tcboxedraster}\par
\begin{tcboxedraster}[raster columns=4, raster valign=top]{blankest}
...
\begin{tcboxedraster}[raster columns=2, raster equal height=rows, raster force size=false]{blankest}
...
\begin{tcboxedraster}[raster columns=2, raster equal height=rows]{blankest}
  \begin{tcblisting}{listing only}
\usepackage{memoize}
\mmzset{recompile}
  \end{tcblisting}
...
\begin{tcboxedraster}[raster columns=2, raster valign=top, raster column skip=4mm]{blankest}
...
\begin{tcboxedraster}[raster columns=5, raster valign=top, raster column skip=4mm]{blankest}
...
\begin{tcolorbox}[title=For {\hypercolor{url}{white}\Emacs} users]
...
\begin{tcboxedraster}[raster columns=3, raster force size=false, raster valign=top]{blankest}
...
\begingroup
\mmzset{disable}
\begin{tcboxedraster}[raster equal height]{blankest}
...
\paragraph*{The challenges of the vertical capture mode} can sometimes be
addressed relatively easily using options \refmmz{before vbox} and
\refmmz{after vbox}.  Below, it is shown how the vertical spacing around
memoized \env{tcolorbox}es in this manual was made (almost) identical to the
unmemoized situation.%
\footnote{This code can be found \reffile{memoize-doc.sty}, the style file used
  for this manual, which also contains settings for dealing (more of less
  successfully) with breakable and floating boxes, equal height groups, and
  enumeration of \env{tcolorbox}es.}%

The issue we are facing stems from the fact that a \env{tcolorbox} itself
controls the amount of vertical space around it (ultimately using keys |before|
and |after|).  One part of the issue is that this vertical space gets baked
into the extern (well, partially so: the space at the top actually disappears,
set \refmmz{padding}|=0pt| to see this clearly).  The other part is that when
the extern is utilized, \env{tcolorbox} cannot add the right amount of space
around it, simply because it is never executed --- this is particularly
unfortunate if the surrounding space was set to stretch or shrink.

The solution is to first prevent any space getting baked into the extern (a job
for \refmmz{memoized code suffix}), and then add the correct amount of space
when the extern is utilized (using \refmmz{before vbox} and \refmmz{after vbox}).

In broad strokes, we define style |tcb options| (lines~2--24), which is then
included among the among the \refmmz{auto}-options of the various environments
offered by package \pkg{tcolorbox} (lines~25--27).\cprotect\footnote{In the
  code, the style is applied by writing |options=tcb options|; this is
  equivalent to but executes a bit faster than plain |tcb options|.}  We now
turn to the definition of the style.  First of all, we set
\refmmz{capture}|=|\refmmz{capture=vbox} (line~3) so that the extern is
captured into a vertical box, and later typeset as a vertical box as well.  Key
\refmmz{verbatim} (line~4) is only used because the \env{tcolorbox}es in this
manual contain a lot of code; it is irrelevant for the issue of vertical
spacing.
...
\mmzset{
  tcb options/.style={
    capture=vbox,
    verbatim,
    memoized code suffix=\vskip-\lastskip,
    before vbox=\unexpanded{%
      \begingroup
      ...
      \if@nobreak
        \iftcb@ignorenobreak
          \kvtcb@beforebox
        \else
          \kvtcb@beforebox@nobreak
        \fi
      \else
        \kvtcb@beforebox
      \fi
    },
    after vbox=\unexpanded{%
      \let\iftcb@minipage\if@minipage
      \kvtcb@afterbox
      \endgroup
    },  
  },
  auto={tcolorbox}{memoize, options=tcb options},
  auto={tcboxedraster}{memoize, options=tcb options},
  auto={tcblisting}{memoize, options=tcb options},
}
\end{tcblisting}

When the code submitted to memoization is compiled in order to produce an
extern, it is surrounded by \refmmz{memoized code prefix} and \refmmz{memoized
  code suffix}.  In other words, the code submitted to these hooks ``gets into
the extern'' and can be therefore used to change the vertical space around
whatever is typeset by the submitted code.  In our case, no space gets above
the \env{tcolorbox}, so \refmmz{memoized code prefix} may remain empty; we get
rid of the space below the box by setting (line~5) \refmmz{memoized code
  suffix} to (hardcore \hologo{TeX}) |\vskip-\lastskip|.

To have a utilized extern surrounded by the same amount of vertical space as
the original \env{tcolorbox}, we copy--paste--adjust some code from package
\pkg{tcolorbox} itself into the value of \refmmz{before vbox} and \refmmz{after
  vbox} (lines~8--17, 20--21).  In a nutshell, we have to execute macros
|\kvtcb@beforebox| and |\kvtcb@afterbox|, but there are some complications: we
have to define an internal \pkg{tcolorbox} macro (|\iftcb@minipage|), %>\fi
and take care to execute an ``alternative'' |\kvtcb@beforebox| at the start of
a section.  The values of \refmmz{before vbox} and \refmmz{after vbox} start
and finish by |\begingroup| (line~7) and |\endgroup| (line~22), respectively,
because the deployed internal macros change some paragraph-related settings.
Finally, these values are wrapped in |\unexpanded| because the code submitted
to these hooks is fully expanded before it is used.
...
The bridge macro is shown below, and it is less complicated than it might
appear at first sight.  In line~2, we use \refcmd{AdviceIfArgs} to see whether
the argument structure of the handled command was given by the user (if it was,
it is available in macro \refcmd{AdviceArgs}).  If it wasn't, we assume that
the handled command was defined using \cs{NewDocumentCommand} or similar, and
use a code-level \hologo{LaTeX3} command |\cmd_arg_spec:N| to retrieve it and
store it into \refcmd{AdviceArgs} (this part of the code is omitted, as
signaled by the ellipsis in line~3).

\tcbinputexample{_advice-CollectArgumentsRaw}[.excerpt]{%
  listing options app={linerange={-3,15-}},
  listing only, no .listing, no attachment,
  listing options app={numbers=left, numberstyle=\tiny, numbersep=0.5em},
  title={The definition of the initial collector},
}

Lines~5--15 are somewhat of an expansion mess, because we have to construct the
...
\begin{enumerate}
\item The raw options (lines 7--11):
  \begin{enumerate}
  \item In line 7, the advised command's control sequence is designated as the
  ...
  \item In line 8, we add any \refmmzauto{raw collector options} set by Advice
  ...
  \item In lines 9--11, we add the user-given \refmmzauto{collector options},
    if there are any, embedded under \refcmd{collargsSet}.
  \end{enumerate}
\item The argument specification (line 13).
\item The \refmmzauto{inner handler} (line 14).
\end{enumerate}
...
\label{sec:ref:memoization:context}

\begin{doc}{
    key={name=context, par=\meta{tokens}, easy, desc={cumulative, glocal,
        no default, initially set by \refmmz{padding to context}}},
  }
  This key appends the given \meta{tokens} to the \emph{context expression}.
  Memoized code gets recompiled whenever the expansion of the context
  expression changes.
  ...
  yielding the \emph{context value}, whose md5 sum forms the \meta{context md5
    sum} part of the filename of the cc-memo and the extern.  (Use key
  \refmmz{trace} to have the context information (expression, value and md5
  sum) printed to the terminal.)

  This key manipulates a glocal token register (see the frame below) consisting
  of the base token register \docaux{cmd}{mmzContext} and the extra token
  register \docaux{cmd}{mmzContextExtra}.  The user may change the contents of
  these registers directly, as long as they conform to the requirement of
  changing \refcmd{mmzContext} locally and only outside memoization, and
  changing \refcmd{mmzContextExtra} globally and only during memoization.
\end{doc}

\begin{tcolorbox}[title=Glocal variables, label=frame:glocal]
  For some variables (in this frame, let ``variable'' be understood as a macro
  or a token register), it makes sense that they are manipulated both outside
  and during memoization process.  For example, font size is best added to
  \refmmz{context} in the preamble (so outside memoization), so that the
  setting affects all memoized code, but a \refcmd{ref} or a citation command
  inside memoized code can only add its key to the context during memoization.
  This creates an issue with locality.  On one hand, it is often crucial that
  changes to a variable are local, i.e.\ that they respect \hologo{TeX}
  grouping.  On the other hand, manipulating the variable from within the
  memoized code locally would not have the desired effect: memoization, here
  understood narrowly as typesetting of the memoized code into the extern box,
  happens in the \hologo{TeX} group introduced by the box (and furthermore, the
  memoized code itself might open arbitrary groups), but the value of variables
  like \refmmz{context} is needed just after memoization, so outside this
  group.  During memoization, manipulation of variables such as
  \refmmz{context} is best global so that it survives until the end of
  memoization.  What are we to do, given that mixing local and global
  assignment to the same variable is not advisable?
  
  A \Emph{glocal} variable is a pair of variables working in tandem to resolve
  this issue.  The ``base'' variable of the pair (for \refmmz{context},
  \refcmd{mmzContext}) should be manipulated only outside memoization, and the
  changes should always be local.  The ``extra'' variable (for
  \refmmz{context}, \refcmd{mmzContextExtra}), which is cleared before each
  memoization, should be manipulated only during memoization, and the changes
  should always be global.  Memoize's keys manipulating glocal variables
  automatically conform to this requirement.

  The value of the combined glocal variable at the end of memoization is the
  concatenation of the base and the extra variable, in this order.
\end{tcolorbox}

\begin{doc}{
    key={name=clear context, easy},
  }
  This key clears the context expression.  More precisely, it locally clears
  \refcmd{mmzContext} outside memoization, and it globally clears
  \refcmd{mmzContextExtra} during memoization.
\end{doc}
...
\begin{doc}{
  ...
  These keys (glocally) append the definition of the given construct to the
  context.
...
\begin{doc}{key={name=padding to context,desc=style}}
  This key (glocally) appends the values of the \refmmz{padding} keys to the
  context, causing the memoized code to be recompiled whenever the padding
  values are changed.  This key is used to initialize \refmmz{context}, so the
  author normally shouldn't have to use this key.
\end{doc}
...
\begin{doc}{
  ...
  While using \refmmz{context} instead of \refmmz{salt} would most often yield
  the same result, one example where only the salt can do the job is presented
  in the Beamer section~\ref{sec:per-overlay}.

  Note that the salt keys manipulate a normal local token register, not a
  glocal token register like \refmmz{context}.  Using the salt keys during
...
\begin{doc}{
  ...
  context (more precisely, it is \meta{reference key}|=|\marg{current value}
  which is appended), making sure that the memoized code is recompiled when the
  reference changes.
...
\label{sec:ref:memoization:advanced}
...
\begin{doc}{
  ...
  thereby forcing the author to use Memoize.  Please open a
  \href{https://github.com/sasozivanovic/memoize/issues}{Github issue} if you
...
\begin{doc}{
  ...
  This key determines whether the \emph{default} memoizaton \refmmz{driver}
  captures the typeset result of the memoized code in a horizontal
  (\refmmz{capture=hbox}, the default) or a vertical (\refmmz{capture=vbox})
  box.

  Using this key |\let|s control sequence \docaux{cmd}{mmzCapture} to either
  \refcmd{hbox} or \refcmd{vbox}, which may (but does not have to) be used by a
  user- or package-defined memoization \refmmz{driver}.  The user may also
  |\let| \refcmd{mmzCapture} to \refcmd{hbox} or \refcmd{vbox} directly.

  When a driver that honors this key is in effect, the value of the key also
  indirectly determines what code will be inserted before and after the extern
  box when the extern is typeset (both in memoization and utilization), see
  \refmmz{before hbox} and friends for further information.
\end{doc}

\begin{doc}{
    key={name=before hbox, par={\meta{code}},
      desc={no default, initially \code{\cs{quitvmode}}}},
    key={name=after hbox, par={\meta{code}},
      desc={no default, initially empty}},
    key={name=before vbox, par={\meta{code}}, desc={no default,
        initially \code{\cs{vskip}\cs{glueexpr}\refcmd{mmzTopSep}\cs{relax}}}},
    key={name=after vbox, par={\meta{code}}, desc={no default,
        initially \code{\cs{vskip}\cs{glueexpr}\refcmd{mmzBottomSep}\cs{noindent}}}},
  }
  These keys set the code which will be inserted before and after the extern
  box when typesetting the extern.  This applies to both memoization (when
  typesetting the extern into its regular position after having externalized it
  into an extern page) and utilization (when including the extern file).

  \begin{tcolorbox}[warning]
    For \hologo{TeX}nical reasons,\footnotemark\ the values of these keys are
    fully expanded before inserting them around the extern box.  In many cases,
    one therefore needs to enclose (parts of) the given code by
    \cs{unexpanded}.
  \end{tcolorbox}\footnotetext{The memoization group is closed before inputting
    a cc-memo (the frame in section~\ref{frame:grouping} explains why), which
    includes the externs.  The contents of \refcmd{mmzBeforeHBox} and friends
    therefore have to be smuggled out of the memoization group.  However, these
    macros may refer to other settings (and the |vbox| macros do, namely to
    \refmmz{mmzTopSep} and \refmmz{mmzBottomSep}), so there is no principled
    way of knowing what else should be smuggled out of the group.  The problem
    is resolved at the cost of user's inconvenience: \refcmd{mmzBeforeHBox} and
    friends are expanded inside the memoization group, and it is the expansion
    which is smuggled out.}
  
  Keys \refmmz{before hbox} and \refmmz{after hbox} set the code which will be
  inserted around an extern captured into a \refcmd{hbox}.  The initial
  settings ensure that the extern box is typeset in the horizontal
  mode.\footnote{This works even in \hologo{XeTeX}, where the
    \refcmd{quitvmode} primitive is called \refcmd{leavevmode}, because Memoize
    defines the former as an alias for the latter.}

  Keys \refmmz{before vbox} and \refmmz{after vbox} set the code which will be
  inserted around an extern captured into a \refcmd{vbox}.  The initial
  settings ensure that the extern box is typeset in the vertical mode, and
  surround it by a configurable amount of vertical glue (see \refmmz{top sep}
  and \refmmz{bottom sep}).

  Instead of using these keys, the user may also directly set the underlying
  macros \docaux{cmd}{mmzBeforeHBox}, \docaux{cmd}{mmzAfterHBox},
  \docaux{cmd}{mmzBeforeVBox} and \docaux{cmd}{mmzAfterVBox}.
\end{doc}

\begin{doc}{
    key={name=top sep, easy, par={\meta{glue}}, desc={no default, initially \code{0pt}}},
    key={name=bottom sep, easy, par={\meta{glue}}, desc={no default, initially \code{0pt}}},
  }
  These keys set the amount of vertical glue inserted before and after an
  extern typeset in the vertical mode, assuming that the initial settings of
  keys \refmmz{driver}, \refmmz{before vbox} and \refmmz{after vbox} are in
  effect.

  Under the hood, these keys store the given \meta{glue} into macros
  \docaux{cmd}{mmzTopSep} and \docaux{cmd}{mmzBottomSep}.  The user may
  redefine these macros directly.
\end{doc}

\begin{doc}{
    par=\meta{code}, desc={cumulative, initially empty},
    key={name=at begin memoization},
    key={name=at end memoization},
    key={name=after memoization},
    key={name=memoized code prefix},
    key={name=memoized code suffix},
  }
  These keys append \meta{code} to various memoization hooks.  Under the hood,
  the \meta{code} is appended to the underlying macros as shown below.  Note
  that three of the hooks are implemented as a glocal variable (see the frame
  in section~\ref{frame:glocal}) and therefore correspond to a pair of macros
  (a base and an extra macro).

  \begin{itemize}
    \item \refmmz{at begin memoization}: \docaux{cmd}{mmzAtBeginMemoization}
    \item \refmmz{at end memoization}:
      \docaux{cmd}{mmzAtEndMemoization} and \docaux{cmd}{mmzAtEndMemoizationExtra}
    \item \refmmz{after memoization}:
      \docaux{cmd}{mmzAfterMemoization} and \docaux{cmd}{mmzAfterMemoizationExtra}
    \item \refmmz{memoized code prefix}: \docaux{cmd}{mmzMemoizedCodePrefix}
    \item \refmmz{memoized code suffix}:
      \docaux{cmd}{mmzMemoizedCodeSuffix} and \docaux{cmd}{mmzMemoizedCodeSuffixExtra}
  \end{itemize}

  The macros listed above may also be redefined directly by the
  user,\footnote{Use \cs{appto}, \cs{eappto}, \cs{gappto} and \cs{xappto} of
    package \pkg{etoolbox} (loaded by Memoize) to easily append code to these
    macros.}  as long as one keeps to the requirement of redefining the base
  macros (without ``\texttt{Extra}'') locally and only outside memoization, and
  redefining the extra macros (with ``\texttt{Extra}'') globally and only
  during memoization (with the term ``memoization'' here understood narrowly as
  the compilation of the code submitted to memoization).

  The keys automatically conform to the above requirement, and may therefore be
  used both outside and during memoization.  In the former case, their effect
  is local to the current group; in the latter case, the code given to
  \refmmz{at begin memoization} and \refmmz{memoized code prefix} is executed
  immediately, while the assignment performed by the other keys is global, so
  that the changes surely survive until the end of memoization.
  ...
  \item Initialize various conditionals, macros and token registers.  (Here is
    where the ``\texttt{Extra}'' hooks are cleared.)  Note that at this point,
    we're inside a ``memoization group'' opened for \refcmd{Memoize} (e.g.\ by
    the automemoization handler).
  \item Execute \refmmz{at begin memoization} hook, i.e.\ the contents of
    macro \refcmd{mmzAtBeginMemoization}.
  \item Execute the memoization \refmmz{driver} (which normally compiles the
    given code in another \hologo{TeX} group), which receives:
    \begin{itemize}
    \item the contents of \refmmz{memoized code prefix} hook, i.e.\ macro
      \refcmd{mmzMemoizedCodePrefix},
    \item the code being memoized,
    \item the contents of \refmmz{memoized code suffix} hook, i.e.\ macros
      \refcmd{mmzMemoizedCodeSuffix} and \refcmd{mmzMemoizedCodeSuffixExtra} (in
      this order).
    \end{itemize}
  ...
  \item Execute \refmmz{after memoization} hook, i.e.\ the contents of macros
    \refcmd{mmzAfterMemoization} and \refcmd{mmzAfterMemoizationExtra} (in this
    order).  (The contents of \refcmd{mmzAfterMemoization} are smuggled of the
    memoization group, ensuring that any changes to this macro within the
    memoization group, e.g.\ by \refmmz{auto}-options, are respected.)
  \end{enumerate}
\end{doc}
...
\begin{doc}{
  ...
  ships with a \refcmd{tikz} collector, \docaux{cmd}{AdviceCollectTikZArguments}, which
  ignores them completely, as it knows everything about the idiosyncrasies of
  that command anyway.  Incidentally, \refcmd{AdviceCollectTikZArguments} becomes
...
\begin{doc}{easy,
  ...
  argument specification automatically.
  ...
  collector (\refcmd{AdviceCollectTikZArguments}).
