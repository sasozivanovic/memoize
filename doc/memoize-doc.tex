...
\begin{doc}{
  ...
  \begin{center}
    \begin{tabular}{rl}
      ...
      &\code{l\&\&\braces{\cs{mmzBiblatexMarkCitationKey}}m}\\
      \refmmzauto{volcite}, \refmmzauto{volcites}
      &\code{lm l\&\&\braces{\cs{mmzBiblatexMarkCitationKey}}m}\\
    \end{tabular}
  \end{center}
  
  Looking at \refmmzauto{cite}, |l| grabs everything up to the first opening
  brace (and will receive the optional star, prenote and postnote), while |m|
  receives the braced \meta{citation key} argument.  The |&&| construct
  preceding |m| specifies the \refmmzauto{raw collector options} for that
  argument: \docaux{cmd}{mmzBiblatexMarkCitationKey} applies an argument
  processor storing the citation key in an internal macro.  Key
  \refmmzauto{volcite} precedes \refmmzauto{cite}'s \refmmzauto{args} by |lm|,
  grabs the optional arguments and the volume; note that the citation key is
  the second braced argument of |\volcite| and friends.  The two
  \refmmzauto{cites} variants iterate the given \refmmzauto{args} as long as
  the citation arguments continue.
  ...
  by \code{\refmmz{auto}=\braces{%
        \refmmzauto{cite},
        \refmmzauto{args}=l\&\&\braces{\cs{mmzBiblatexMarkCitationKey}}m lm%
      }%
    }.
\end{doc}
