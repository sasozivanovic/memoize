...
\refmmz{salt} has a similar effect as \refmmz{context}: it contributes towards
...
and the memoized code.  (As the code md5 sum is calculated prior to
memoization, using \refmmz{salt} and friends during memoization has no effect.)
...
salt, the two frames would create the same number of overlays: two, the number
of overlays created by the final frame.
